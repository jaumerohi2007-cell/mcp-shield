...
export async function runUninstallCommand(targetApp: string): Promise<void> {
  ...
  try {
    ...
    if ((err as NodeJS.ErrnoException).code === 'ENOENT') {
      fail(`Could not find config file at ${configPath}.`);
    }
    fail(`cannot read ${configPath}: ${err instanceof Error ? err.message : String(err)}`);
  }
  ...
  if (raw !== null) {
    try {
      ...
      fail(`${configPath} is not valid JSON.`);
    }
  }

  const existingServers = config['mcpServers'];
  if (!existingServers || typeof existingServers !== 'object' || Array.isArray(existingServers)) {
    fail(`No "mcpServers" found in ${configPath}.`);
  }
  ...
  for (const [serverName, serverConfig] of Object.entries(servers)) {
    ...
    // Check if it's protected by mcp-shield
    if (serverConfig.command === 'mcp-shield' || (typeof serverConfig.command === 'string' && serverConfig.command.includes('mcp-shield'))) {
      const args = serverConfig.args || [];
      const dashDashIndex = args.indexOf('--');
      
      if (dashDashIndex !== -1 && args.length > dashDashIndex + 1) {
        // Restore original command and args
        serverConfig.command = args[dashDashIndex + 1];
        serverConfig.args = args.slice(dashDashIndex + 2);
        
        // Clean up empty args array
  ...
  if (modifiedCount === 0) {
    process.stderr.write(`[mcp-shield] No servers were protected by mcp-shield in ${targetApp}.\n`);
    return;
  }
  ...
  const message = `[mcp-shield] Successfully UNINSTALLED from ${modifiedCount} server(s) in ${targetApp}.`;
  const line = process.stderr.isTTY ? `\x1b[32m${message}\x1b[0m\n` : `${message}\n`;
